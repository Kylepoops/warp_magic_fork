    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (sender instanceof Player) {
            ...
            if (!playersOnCooldown.containsKey(playerID)) {
                if (args.length > 0) {
                    ...
                    if (target != null) {
                        if (target != player) {
                            ...
                            target.sendMessage(ChatColor.GREEN + Message.REQUEST_TPA.fromPlayer(player.getDisplayName()));
                            ...
                            if (PluginConfig.getTpRequestCooldown() > 0) {
                                playersOnCooldown.put(playerID, new Cooldown(PluginConfig.getTpRequestCooldown(), () -> {
                                    playersOnCooldown.remove(playerID);
                                }));
                            }
                ...
                Cooldown cooldown = playersOnCooldown.get(playerID);
